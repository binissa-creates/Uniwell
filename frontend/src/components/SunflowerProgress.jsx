    ...
    <div className="flex flex-col items-center gap-4 group perspective-1000">
      <style>{`
        ...
        @keyframes pulseAura {
          ...
          50% { opacity: ${auraOpacity + 0.15}; transform: scale(${auraScale + 0.05}); }
        }
        @keyframes orbit {
      ...
      <div className="relative w-36 h-36 flex items-center justify-center">
        ...
        <div className="absolute inset-0 z-20">
          <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90" style={{ animation: 'ringRotate 60s linear infinite' }}>
            {/* Background Track */}
            <circle cx="50" cy="50" r={radius} fill="none" stroke="#fde8e0" strokeWidth="4" />
            ...
            <circle
              ...
              style={{ filter: 'drop-shadow(0 0 5px rgba(246,201,69,0.4))' }}
            />
            ...
            <circle
              ...
              strokeWidth="2"
              strokeLinecap="round"
              strokeDasharray={`${circum / 6} ${circum}`}
              style={{ 
                opacity: 0.4,
        ...
        <div className="relative z-30 flex flex-col items-center" style={{ animation: 'float 5s ease-in-out infinite' }}>
          
          {/* Royal Crown (100+ Days) */}
          {isRoyal && (
            <div className="absolute -top-10 scale-75" style={{ animation: 'crownHover 3s ease-in-out infinite' }}>
          ...
          <div style={{ animation: 'sway 6s ease-in-out infinite', transformOrigin: 'bottom center' }}>
            {/* Sunflower Graphic */}
            <svg width="64" height="64" viewBox="0 0 100 100" className="overflow-visible">
              {/* Stem (Now behind the flower) */}
              <path d="M50 55 Q50 80 45 100" stroke="#b0cfad" strokeWidth="3" fill="none" strokeLinecap="round" />
              
              {/* Aesthetic Leaf */}
              <path 
                d="M48 85 Q60 75 55 65 Q45 75 48 85" 
                fill="#b0cfad" 
                opacity="0.9" 
                transform="rotate(-10 48 85)"
              />

              {/* Petals (Back Layer for volume) */}
              <g style={{ animation: 'breathe 4s ease-in-out infinite', transformOrigin: 'center' }}>
                {[...Array(12)].map((_, i) => (
                  <path
                    key={`back-${i}`}
                    ...
                    opacity="0.7"
                    transform={`rotate(${i * 30 + 15} 50 50) scale(0.9)`}
                  />
                ))}
              ...
              <circle cx="50" cy="51" r="18" fill="rgba(0,0,0,0.1)" />
              ...
              <circle 
                ...
                style={{ 
                  filter: isLegendary ? 'drop-shadow(0 0 6px rgba(246,201,69,0.4))' : 'none' 
                }}
              />
              
              {/* Seed texture details (Golden ratio spiral effect) */}
              {[...Array(16)].map((_, i) => {
                const angle = i * 137.5 * (Math.PI / 180);
                const r = 3 + Math.sqrt(i) * 2.5;
                return (
                  <circle 
                    key={i} 
        ...
        {/* Orbiting Sparkles */}
        {isLegendary && [...Array(isRadiant ? 8 : 4)].map((_, i) => (
          <div 
            key={i}
            className="sparkle"
            style={{ 
        ...
        {[...Array(5)].map((_, i) => (
          <div 
            ...
            style={{ 
              ...
              bottom: '20%',
              animation: `driftUp ${2 + Math.random() * 2}s ease-out infinite`,
      ...
      <div className="text-center">
        {isLegendary && (
          <p className="font-playfair italic text-sm text-[#6B5A10] mb-0.5 animate-fadeIn">
            {isRoyal ? 'Royal Bloom' : isRadiant ? 'Radiant Sun' : 'Legendary Bloom'}
          </p>
        )}
        <h4 className="font-jakarta font-bold text-[#3a2b25] text-xl tracking-tight leading-tight">
          {streak} Day Streak
        </h4>
        <p className="font-jakarta text-[11px] font-medium text-[#AA8E7E] mt-1 italic leading-relaxed">
